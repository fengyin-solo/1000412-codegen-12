...
LIST_FIELDS = ["泵站编号", "泵组台数", "运行泵号", "出水流量", "液位高度", "运行电流", "值守人员", "泵站状态"]
STATUSES = ["待启泵", "运行中", "待检修", "已停泵"]


@router.get("", response_model=PageResult[dict])
def list_entries(
    keyword: str | None = Query(default=None, description="按泵站编号检索"),
    status: str | None = Query(default=None, description="待启泵、运行中、待检修、已停泵"),
    page: int = 1,
    size: int = 20,
) -> PageResult[dict]:
    """按泵站编号与状态过滤泵站运行列表；没有数据时返回空页，不报错。"""
    if size > 200:
...
def run_action(entry_id: int, payload: EntryPayload) -> ActionResult:
    """对单条泵站执行启泵运行、安排检修、停泵；不允许的动作会被拦下并说明原因。"""
